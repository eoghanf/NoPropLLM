...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train model on CIFAR-10 dataset using backpropagation",
    ...
    parser.add_argument(
        ...
        type=str, 
        default=None,
        help="Path to YAML configuration file (default: experiment_configs/cifar10backpropagation.yaml)"
    )
    ...
    if args.list_configs:
        ...
        if configs:
            for config in configs:
                print(f"  - {config}")
            print(f"\nUse with: python train_cifar10.py --config experiment_configs/<name>.yaml")
        else:
